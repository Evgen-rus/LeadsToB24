...
Программа собирает данные о лидах из API поставщика данных у которого есть только POST, обрабатывает их, сохраняет в базу данных SQLite и маршрутизирует по клиентам в соответствии с тегами. Каждый клиент может получать данные в свою Google Таблицу и/или через вебхук в CRM.

## Общая схема работы

1. Система регулярно проверяет наличие новых строк в исходной Google Таблице
2. Данные сохраняются в локальную базу данных SQLite из них извлекаются данные (ID, телефон, тег проекта)
3. На основе тега система определяет, какому клиенту принадлежит лид
4. Данные отправляются в таблицу клиента и/или через вебхук в CRM
5. Проверяются на дубликаты только в рамках одного проекта, если находятся дубли cj[hfyz.nczТребования
